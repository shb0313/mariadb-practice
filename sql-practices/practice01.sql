...
select gender, count(*) as '직원 수' from employees 
	where gender = 'm';
select gender, count(*) as '직원 수' from employees 
	where gender = 'f';
select gender, count(gender) as '직원 수' from employees
	group by gender;


-- 문제4.
-- 현재(to_date='9999-01-01', 근무하고 있는 직원 수는 몇 명입니까? (salaries 테이블을 사용합니다.) 
select count(*) as '현재 직원 수' from salaries
...
select count(*) as '부서 매니저' from dept_manager
	where to_date = '9999-01-01';
...
select count(*) as '사원 수' from salaries
...
select count(*) as 'Engineer' from titles
