...
select a.emp_no, a.first_name, b.title, c.salary, d.dept_no, e.max_avg_sal
	from employees a, titles b, salaries c, dept_emp d, (select a.dept_no, max(a.avg_salary) as max_avg_sal
															from (select a.dept_no, avg(b.salary) as avg_salary
																	from dept_emp a, salaries b
																	where a.emp_no = b.emp_no
																		and a.to_date = '9999-01-01'
																		and b.to_date = '9999-01-01'
																	group by dept_no) a) e
    where a.emp_no = b.emp_no
		and b.emp_no = c.emp_no
		and c.emp_no = d.emp_no
		...
		and c.to_date = '9999-01-01'
		and d.to_date = '9999-01-01'
		and c.salary > e.max_avg_sal
    order by c.salary desc;  

-- 문제6.
-- 평균 급여가 가장 높은 부서는?
-- 부서이름, 평균급여 
select a.dept_name, max(b.avg_salary) as 평균급여
...
-- 평균 급여가 가장 높은 직책?
-- 직책, 평균급여 
select a.title, max(b.avg_salary) as 평균급여
	from titles a,(select b.emp_no, avg(b.salary) as avg_salary
							from dept_emp a, salaries b
							where a.emp_no = b.emp_no
								and a.to_date = '9999-01-01'
								and b.to_date = '9999-01-01'
